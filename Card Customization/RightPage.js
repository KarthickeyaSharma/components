...
class RightPage extends Component {
  ...
  componentDidMount() {
    if (getCustomizationItem(global.skuCName).isEmpty()) {
      ...
      if (skuData.rightPage == '') {
        ...
        this.setState({
          jsonData: JSON.parse(skuData.rightPage)[0].pdc_json,
        });
      }
    }
  ...
  onMessage(data) {
    ...
    if (getCustomizationItem(global.skuCName).isEmpty()) {
      ...
      updateRightItem(skuCName, JSON.stringify(pdc_data));
    }
    if ('navigateTo' in mData) {
      if (mData.navigateTo == 'BackPage') {
  ...
  layoutDesign = indexPos => {
    AsyncStorage.setItem('layOut', JSON.stringify(indexPos));
    this.setState({
      jsonData:
        indexPos === 0
  ...
  alignWebText = (alignment,label) => {
    AsyncStorage.setItem('fAlign', label);
    var imsgs = `
        (function () {
  ...
  fontWebText = font_fam => {
    AsyncStorage.setItem('fontfam', font_fam);
    var imsgs = `
        (function () {
            $("#fontme").val("${font_fam}");
            $("#applyfont").click();
  ...
  fontSizeText = (font_size,fontlabel) => {
    // alert(fontlabel); 
    AsyncStorage.setItem('fSize', fontlabel);
    this.setState({
      font_Size: font_size,
    });
    var imsgs = `
              ...
              <WebView
                ...
                source={{
                  html: `<html>
                      ...
                      <style type="text/css">
                          #nextPage, #prevPage, #canvas2json, #myTextArea, #span, #colorme, #applytext, #applyAlign, #inspireme, #applyfont, #fontme, #applyfontSize, #fontSize, #applykeyBoard, #alterImg, #myMenu {
                              display: none;
                          }
                          .preloader {
                            position: fixed;
                              ...
                              $(function () {
                                ...
                                fabric.Cropzoomimage = fabric.util.createClass(fabric.Image,
                                  ...
                                  $("#nextPage").click(function() {
                                      var json = canvas.toJSON();
                                      var svg = canvas.toSVG();
                                      var datas = { "jsonfile": JSON.stringify(json), "svgfile": JSON.stringify(svg), "navigateTo": "BackPage" };
                                      window.ReactNativeWebView.postMessage(JSON.stringify(datas));
                                  });
                                  ...
                                  $("#prevPage").click(function() {
                                      var json = canvas.toJSON();
                                      var svg = canvas.toSVG();
                                      var datas = { "jsonfile": JSON.stringify(json), "svgfile": JSON.stringify(svg), "navigateTo": "LeftPage" };
                                      window.ReactNativeWebView.postMessage(JSON.stringify(datas));
                                  });
                                  ...
                                  $("#applykeyBoard").click(function() { 
                                    ...
                                    if(canvas.getActiveObject()==null) {
                                      ...
                                      { 
                                        ...
                                        canvas.getActiveObject().borderColor="#EF80B1";
                                          canvas.renderAll(); 
                                      }
                                    }
                                  });
